...
oneflow::cfg::{{ cls.name }} FromProto(const oneflow::{{ cls.name }}& proto_{{ cls.name.lower() }}) {
  oneflow::cfg::{{ cls.name }} cfg_{{ cls.name.lower() }};
...
{% elif util.field_is_enum_type(field) %}
    for (const int& elem : proto_{{ cls.name.lower() }}.{{ util.field_name(field) }}() ) {
      cfg_{{ cls.name.lower() }}.add_{{ util.field_name(field) }}(oneflow::cfg::{{ util.field_enum_name(field) }}(elem));
    }
{% else %}
...
{% for oneof in util.message_type_oneofs(cls) %}
  ...
  oneflow::cfg::{{ cls.name }}::{{ util.oneof_enum_name(oneof) }} {{ util.oneof_name(oneof) }}_case = oneflow::cfg::{{ cls.name }}::{{ util.oneof_enum_name(oneof) }}(int(proto_{{ cls.name.lower() }}.{{ util.oneof_name(oneof) }}_case()));
  switch ({{ util.oneof_name(oneof) }}_case) {
{% for field in util.oneof_type_fields(oneof) %}
    case oneflow::cfg::{{ cls.name }}::{{ util.oneof_type_field_enum_value_name(field) }}: {
{% if util.field_is_message_type(field) %}
      cfg_{{ cls.name.lower() }}.mutable_{{ util.field_name(field) }}()->CopyFrom(FromProto(proto_{{ cls.name.lower() }}.{{ util.field_name(field) }}()));
{% elif util.field_is_enum_type(field) %}
      cfg_{{ cls.name.lower() }}.set_{{ util.field_name(field) }}(Proto{{ util.field_enum_name(field) }}ToCfg{{ util.field_enum_name(field) }}(proto_{{ cls.name.lower() }}.{{ util.field_name(field) }}()));
...
{% endfor %}{# oneof_field #}
    case oneflow::cfg::{{ cls.name }}::{{ util.oneof_name(oneof).upper() }}_NOT_SET: {
      break;
    }
  }
{% endfor %}{# oneofs #}
...
{% if util.field_is_message_type(field)%}
    for (const oneflow::cfg::{{ util.field_type_name(field) }}& elem : cfg_{{ cls.name.lower() }}.{{ util.field_name(field) }}() ) {
