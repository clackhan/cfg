...
oneflow::cfg::{{ util.enum_name(enm) }} Proto{{ util.enum_name(enm) }}ToCfg{{ util.enum_name(enm) }}(const oneflow::{{ util.enum_name(enm) }}& proto_enum) {
  return oneflow::cfg::{{ util.enum_name(enm) }}(int(proto_enum));
}
{% endfor %}{# enms #}

{% for cls in util.module_message_types(module) %}

oneflow::cfg::{{ util.class_name(cls) }} FromProto(const oneflow::{{ util.class_name(cls) }}& proto_{{ util.class_name(cls).lower() }}) {
  oneflow::cfg::{{ util.class_name(cls) }} cfg_{{ util.class_name(cls).lower() }};
...
{% elif util.field_is_enum_type(field) %}
    for (const int& elem : proto_{{ util.class_name(cls).lower() }}.{{ util.field_name(field) }}() ) {
      cfg_{{ util.class_name(cls).lower() }}.add_{{ util.field_name(field) }}(oneflow::cfg::{{ util.field_enum_name(field) }}(elem));
    }
{% else %}
...
{% for oneof in util.message_type_oneofs(cls) %}
  ...
  oneflow::cfg::{{ util.class_name(cls) }}::{{ util.oneof_enum_name(oneof) }} {{ util.oneof_name(oneof) }}_case = oneflow::cfg::{{ util.class_name(cls) }}::{{ util.oneof_enum_name(oneof) }}(int(proto_{{ util.class_name(cls).lower() }}.{{ util.oneof_name(oneof) }}_case()));
  switch ({{ util.oneof_name(oneof) }}_case) {
{% for field in util.oneof_type_fields(oneof) %}
    case oneflow::cfg::{{ util.class_name(cls) }}::{{ util.oneof_type_field_enum_value_name(field) }}: {
{% if util.field_is_message_type(field) %}
      cfg_{{ util.class_name(cls).lower() }}.mutable_{{ util.field_name(field) }}()->CopyFrom(FromProto(proto_{{ util.class_name(cls).lower() }}.{{ util.field_name(field) }}()));
{% elif util.field_is_enum_type(field) %}
      cfg_{{ util.class_name(cls).lower() }}.set_{{ util.field_name(field) }}(Proto{{ util.field_enum_name(field) }}ToCfg{{ util.field_enum_name(field) }}(proto_{{ util.class_name(cls).lower() }}.{{ util.field_name(field) }}()));
...
{% endfor %}{# oneof_field #}
    case oneflow::cfg::{{ util.class_name(cls) }}::{{ util.oneof_name(oneof).upper() }}_NOT_SET: {
      break;
    }
  }
{% endfor %}{# oneofs #}
...
oneflow::{{ util.class_name(cls) }} ToProto(const oneflow::cfg::{{ util.class_name(cls) }}& cfg_{{ util.class_name(cls).lower() }}) {
...
{% if util.field_is_message_type(field)%}
    for (const oneflow::cfg::{{ util.field_type_name(field) }}& elem : cfg_{{ util.class_name(cls).lower() }}.{{ util.field_name(field) }}() ) {
